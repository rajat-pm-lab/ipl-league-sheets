...
export function computeWeeklyScores(predictions, matchResults, rules, weekNum, weeklyOverrides = {}) {
  ...
  return PLAYERS.map((player) => {
    ...
    // Special week mechanics (Double Dip and Hate Team)
    ...
    for (const match of weekMatches) {
      ...
      // Corner case: double dip game cannot be a hate team game
      const isDoubleDip = doubleDipMatch === match.matchNum && !hateTeamPlaying;

      // Confidence scoring (Week 3+): overrides standard/DD/HT for that match
      const confidence = playerPicks._confidence?.[match.matchNum];

      if (isNR) {
        // No Result — standard for all mechanics
        draws++;
        points += nrPts;
      } else if (confidence !== undefined) {
        // Confidence scoring: correct = +10 + confidence, wrong = -confidence
      ...
      } else if (hateTeamPlaying) {
        // Hate Team match: forced pick against hate team
        // Hate team loses → +15 (+10 correct + 5 bonus)
        // Hate team wins  → -5
        ...
        // BAU: +correctPts for correct, 0 for wrong
        if (pick === match.winner) {
          wins++;
          points += correctPts;
        } else {
